...
def add_test_simulation_type():
    new_simulation_type = SimulationType(
        name="Test Simulation", description="testdescription"
    )
...
def test_delete_simulation_type():
    ...
    with app.app_context():
        ...
        assert executed == {
            "data": {"deleteSimulationType": {"ok": "Simulation Type Deleted"}}
        }
        end_test(db_fd)
...
def test_delete_vehicle():
    ...
    with app.app_context():
        ...
        assert executed == {"data": {"deleteVehicle": {"ok": "Vehicle Deleted"}}}
        add_test_vehicle()
        executed = client.execute(
            """
            mutation {
...
def test_create_simulation_config():
    ...
    with app.app_context():
        ...
        executed = client.execute(
            ...
            + environmental_conditions.replace('"', '\\"')
            + """\",
                    initialConditions: \""""
            + initial_conditions.replace('"', '\\"')
            + """\"
                    physicalConstants: \""""
            + physical_constraints.replace('"', '\\"')
            + """\"
                    timeSettings: \""""
            + time_settings.replace('"', '\\"')
            + """\"
                    trafficRules: \""""
            + traffic_rules.replace('"', '\\"')
            + """\"
                    successDefinition: \""""
            + success_definition.replace('"', '\\"')
            + """\"
                ) {
                    ok
                    simulationConfig {
        ...
        executed = client.execute(
            ...
            mutation {
                updateSimulationConfig(
                    ...
                    simulationTypeId: "2",
                    environmentalConditions: \""""
            + new_environmental_conditions.replace('"', '\\"')
            + """\",
                    initialConditions: \""""
            + new_initial_conditions.replace('"', '\\"')
            + """\",
                    physicalConstants: \""""
            + new_physical_constraints.replace('"', '\\"')
            + """\",
                    timeSettings: \""""
            + new_time_settings.replace('"', '\\"')
            + """\",
                    trafficRules: \""""
            + new_traffic_rules.replace('"', '\\"')
            + """\",
                    successDefinition: \""""
            + new_success_definition.replace('"', '\\"')
            + """\"
                ) {
                    ok
                    simulationConfig {
...
def test_delete_simulation_config():
    ...
    with app.app_context():
        ...
        assert executed == {
            "data": {"deleteSimulationConfig": {"ok": "Simulation Config Deleted"}}
        }
        end_test(db_fd)
...
def test_create_simulation_result():
    ...
    with app.app_context():
        ...
        executed = client.execute(
            ...
            + """\",
                    navigationData: \""""
            + navigation_data.replace('"', '\\"')
            + """\",
                    safetyMetrics: \""""
            + safety_metrics.replace('"', '\\"')
            + """\",
                    vehicleSystemPerformance: \""""
            + vehicle_system_performance.replace('"', '\\"')
            + """\"
                ) {
                    ok
                    simulationResult {
...
def test_update_simulation_result():
    ...
    with app.app_context():
        ...
        executed = client.execute(
            ...
            + """\",
                    navigationData: \""""
            + new_navigation_data.replace('"', '\\"')
            + """\",
                    safetyMetrics: \""""
            + new_safety_metrics.replace('"', '\\"')
            + """\",
                    vehicleSystemPerformance: \""""
            + new_vehicle_system_performance.replace('"', '\\"')
            + """\"
                ) {
                    ok
                    simulationResult {
...
def test_delete_simulation_result():
    ...
    with app.app_context():
        ...
        assert executed == {
            "data": {"deleteSimulationResult": {"ok": "Simulation Result Deleted"}}
        }
        end_test(db_fd)
